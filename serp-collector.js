...
const getUniqueResults = async (api, query, city, engine = 'YANDEX') => {
  ...
  while (uniqueUrls.size < CONFIG.collection.targetUniqueResults && page < CONFIG.collection.maxPages) {
    ...
    try {
      ...
      // Если это первая страница и ошибка критичная, пробрасываем её дальше
      if (page === (engine === 'YANDEX' ? 0 : 1) && uniqueResults.length === 0 && error.code !== 111) {
...
const main = async () => {
  ...
  try {
    ...
    try {
      ...
      console.error(`⚠️ Не удалось получить баланс: ${error.message}`);
    }
